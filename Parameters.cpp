...
namespace Parm
{
	PARM GetParm(int argc, char* argv[]) { // запись значений входных параметров в структуру типа PARM
		...
		for (int i = 1; i < argc; i++) {
			...
			if (strstr(argv[i], PARM_OUT)) { // проверка на наличие параметра -out
				strcpy_s(parm.out, argv[i] + strlen(PARM_OUT));
				out_ = 1; // результат проверки
			}
			if (strstr(argv[i], PARM_LOG)) { // проверка на наличие параметра -log
